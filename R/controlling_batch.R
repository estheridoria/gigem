...
formula_stringss <- c("Sleep_Time_All ~ ", "Sleep_Time_L ~ ", "Sleep_Time_L ~ ")
...
for(i in seq_along(formula_stringss)){
  ...
  for (var in categorical_vars) {
    ...
    if (length(unique(dat[[var]])) > 1) {
      ...
      if (formula_string == formula_stringss[i]) {
  ...
  dataset <- new_data
    ...
    col<- i-1+ncol(dataset)
    dataset[[col]]<- new_data$predicted
    }
